  ...
  connect(opts) {
    ...
    return promise = new Promise((resolve, reject) => {
      ...
      this.socket.on('data', async data => {
        ...
        const parsed = await this._parseData(data);
        if (parsed.event === 'ready') {
          return parsed.shellPrompt;
        }
      })
      ...
      this.socket.write(cmd, () => {
        ...
        if (this.execTimeout) {
          execTimeout = setTimeout(() => {
            ...
            this.removeEventListener('responseready', responseHandler)
            this.removeEventListener('bufferexceeded', buffExcHandler)
      ...
      if (this.socket.writable) {
        this.socket.write(data, () => {
          ...
          if (!this.waitfor || !opts) {
            setTimeout(() => {
              if (response === '') {
                  this.removeEventListener('data', sendHandler)
                  reject(new Error('response not received'))
                  return
              }
          ...
          function sendHandler(data) {
            ...
            if (self.waitfor) {
              ...
              self.removeEventListener('data', sendHandler)
              resolve(response)
            }
          }
        })
  ...
  async _parseData(chunk) {
    ...
    if (this.state === 'getprompt') {
      ...
      if (utils.search(stringData, this.loginPrompt) !== -1) {
        ...
        return {type: 'ready', shellPrompt: this.shellPrompt};
      }
      ...
      if (promptIndex === -1 && stringData.length !== 0) {
        if (utils.search(stringData.replace(/\r?\n|\r/g, ''), this.pageSeparator) !== -1) {
